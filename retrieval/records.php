...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'delete_single' && isset($_POST['record_id'])) {
        ...
        $stmt = $pdo->prepare("SELECT * FROM retrieval_records WHERE record_id = ?");
        $stmt->execute([$record_id]);
        $deleted_record = $stmt->fetch();
        
        if ($deleted_record) {
            $_SESSION['rt_deleted_records'] = [$deleted_record];
            $_SESSION['rt_delete_time'] = time();
            
            $stmt = $pdo->prepare("DELETE FROM retrieval_records WHERE record_id = ?");
            $stmt->execute([$record_id]);
    ...
    if ($_POST['action'] == 'delete_multiple' && isset($_POST['selected_records'])) {
        ...
        if ($count > 0) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM retrieval_records WHERE record_id IN ($placeholders)");
            $stmt->execute($selected_ids);
            $deleted_records = $stmt->fetchAll();
            
            $_SESSION['rt_deleted_records'] = $deleted_records;
            $_SESSION['rt_delete_time'] = time();
            
            $stmt = $pdo->prepare("DELETE FROM retrieval_records WHERE record_id IN ($placeholders)");
            $stmt->execute($selected_ids);
    ...
    if ($_POST['action'] == 'undo_delete' && isset($_SESSION['rt_deleted_records'])) {
        $deleted_records = $_SESSION['rt_deleted_records'];
        $restored_count = 0;
        ...
        foreach ($deleted_records as $record) {
            try {
                $stmt = $pdo->prepare("
                    ...
                    (record_id, college_department, request_type, student_employee_id, last_name, first_name, 
                     middle_name, email_address, password, record_date, recorded_by, remarks, created_at,
        ...
        unset($_SESSION['rt_deleted_records']);
        unset($_SESSION['rt_delete_time']);
...
$show_undo = false;
if (isset($_SESSION['rt_deleted_records']) && isset($_SESSION['rt_delete_time'])) {
    $time_elapsed = time() - $_SESSION['rt_delete_time'];
    if ($time_elapsed <= 30) {
        $show_undo = true;
...
$filter_request_type = isset($_GET['request_type']) ? $_GET['request_type'] : '';
...
// Build query
$sql = "SELECT * FROM retrieval_records WHERE 1=1";
$params = [];
...
if (!empty($filter_request_type)) {
    $sql .= " AND request_type = ?";
    $params[] = $filter_request_type;
}
...
if (!empty($search)) {
    $sql .= " AND (last_name LIKE ? OR first_name LIKE ? OR email_address LIKE ? OR student_employee_id LIKE ? OR college_department LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
...
<head>
    ...
    <title>Retrieval Records - LDCU EdTech System</title>
    ...
    <style>
        ...
        #deleteBar.show { display: flex; }
        #deleteBar span { color: #c53030; font-weight: 600; }
        ...
        .selection-item:hover { background: #edf2f7; }
        ...
        tr.selected-row:hover { background-color: #bee3f8 !important; }
        ...
        .badge-received { background: #c6f6d5; color: #276749; }
        .badge-processing { background: #bee3f8; color: #2b6cb0; }
        .badge-accomplished { background: #c6f6d5; color: #276749; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="container">
        <div class="card">
            <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                <h3 class="card-title">Account/Password Retrieval Records (<?php echo $total_records; ?> total records)</h3>
            ...
            <form method="GET" action="" class="search-bar">
                ...
                <select name="request_type" class="form-control" style="width: auto;">
                    ...
                    <option value="Account" <?php echo $filter_request_type == 'Account' ? 'selected' : ''; ?>>Account</option>
                    <option value="Password" <?php echo $filter_request_type == 'Password' ? 'selected' : ''; ?>>Password</option>
                </select>
                            ...
                            <tr data-record-id="<?php echo $record['record_id']; ?>">
                                <td class="checkbox-cell">
                                    <input type="checkbox" name="selected_records[]" 
                                           ...
                                           data-id="<?php echo htmlspecialchars($record['student_employee_id']); ?>">
                                </td>
            ...
            <div class="pagination">
                ...
                if ($filter_request_type) $query_params['request_type'] = $filter_request_type;
    ...
    <div class="undo-toast" id="undoToast">
        <span><?php echo count($_SESSION['rt_deleted_records']); ?> record(s) deleted</span>
        ...
        <button type="button" class="close-toast" onclick="document.getElementById('undoToast').style.display='none'">x</button>
    </div>
    <script>
    ...
    <script>
        const SelectionManager = {
            storageKey: 'rtSelectedRecords',
            detailsKey: 'rtSelectedRecordDetails',
            ...
            add(id, name, studentId) {
                ...
                details[id] = { name: name, studentId: studentId };
        ...
        document.getElementById('selectAll').addEventListener('change', function() {
            document.querySelectorAll('.record-checkbox').forEach(checkbox => {
                ...
                const row = checkbox.closest('tr');
                if (this.checked) {
                    SelectionManager.add(checkbox.value, name, studentId);
                    if (row) row.classList.add('selected-row');
                } else {
        ...
        document.querySelectorAll('.record-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                if (this.checked) {
                    SelectionManager.add(this.value, name, studentId);
                    if (row) row.classList.add('selected-row');
                } else {
        ...
        function updateSelectionPreview() {
            ...
            for (const [id, record] of Object.entries(details)) {
                html += '<div class="selection-item"><div class="item-info"><div class="item-name">' + record.name + '</div><div class="item-email">' + (record.studentId || 'No ID') + '</div></div><button type="button" class="remove-btn" onclick="removeFromSelection(\'' + id + '\')">x</button></div>';
            }
            list.innerHTML = html;
        }
